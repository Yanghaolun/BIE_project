...
def M(x, y, s):
    xi, eta = r(s)
    dxi, deta = drdt(s)
    return (deta * (x - xi) - dxi * (y - eta)) / (2 * np.pi * ((x - xi) ** 2 + (y - eta) ** 2))
...
plt.contourf(X, Y, np.log(np.abs(Z_exact - Z) + 1e-16) / np.log(10), levels=np.linspace(-17, -1, 17))
plt.axis('equal')
plt.axis('off')
plt.colorbar()
plt.show()
# plt.savefig('./figures/untrivial_naive.pdf', format = 'pdf', bbox_inches = 'tight')